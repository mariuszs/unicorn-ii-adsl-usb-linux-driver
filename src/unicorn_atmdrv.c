/*
  ...
  The chipset consists of the ADSL DMT transceiver ST70138 and ST70174
...
// use the dev_data pointer to store the reassembly buffer when receiving
#define VCC_REAS_BUF(vcc) (((struct sk_buff *) (vcc)->dev_data))
...
static void rcv_aal0(struct unicorn_atmdrv *drv,struct atm_vcc *vcc,unsigned char *cell)
{
	...
	if (skb) {
		...
		if (atm_charge(vcc, skb->truesize)) {
			...
			DBG(ATM_D,"dropping incoming packet,size=%d\n",skb->truesize);
...
static struct sk_buff *rawcell_decode(struct atm_vcc *vcc, unsigned char *cell)
{
	...
	skb = VCC_REAS_BUF(vcc);
	if (!skb) {
		skb = (struct sk_buff *)dev_alloc_skb(max_mru);
		if (!skb) {
			DBG(ATM_D,"dev_alloc_skb failed\n");
			return NULL;
		}
	...
	// copy data
		...
		DBG(ATM_D,"packet too long, skb->len=%d,max_mru=%d\n",skb->len,max_mru);
		skb_trim(skb, 0);
	}
	memcpy(skb_put(skb,ATM_CELL_PAYLOAD), cell+5, ATM_CELL_PAYLOAD);
  	...
  	if (cell[3] & 0x2) {
		...
		//VCC_REAS_BUF(vcc) = NULL;
		vcc->dev_data = NULL;
	} else {
		//VCC_REAS_BUF(vcc) = skb;
		vcc->dev_data = skb;
		skb = NULL;
	}
	return skb;
...
static struct sk_buff *aal5_decode(struct sk_buff *skb) 
{
	...
	pdu_length  = ((length + 47 + 8)/ 48) * 48;
  	...
  	if (skb->len < pdu_length) {
		DBG(ATM_D,"skb too short,skb->len=%d,pdu_length=%d\n",skb->len,pdu_length);
		goto fail;
	}
...
#endif
{
	...
	case ATM_AAL5:
		if ((vcc->vpi == ATM_VPI_UNSPEC) || (vcc->vci == ATM_VCI_UNSPEC)) {
			DBG(ATM_D,"VCC unspecified not supported for AAL%d\n",vcc->qos.aal);
			return -EINVAL;
		}
	case ATM_AAL0:
		break;
...
static void unicorn_atm_close(struct atm_vcc *vcc) 
{
	...
	// freeing resources
	if (VCC_REAS_BUF(vcc)) {
		dev_kfree_skb_any(VCC_REAS_BUF(vcc));
		vcc->dev_data = NULL;
	}
