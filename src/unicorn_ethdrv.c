...
MODULE_DESCRIPTION ("Ethernet driver for the ST UNICORN II ADSL modem.");
...
//#define ATM_VPI_UNSPEC 8
//#define ATM_VCI_UNSPEC 35
...
static void oam_loopback(struct unicorn_ethdrv *drv,unsigned char *cell)
{
	...
	if (!skb) {
		DBG(ATM_D,"dev_alloc_skb of size %d failed\n",ATM_CELL_SIZE);
		return ;
	}
...
static void rcv_oam(struct unicorn_ethdrv *drv,unsigned char *cell,int vpi,int vci)
{
	...
	if (crc != rcvdcrc) {
		DBG(ATM_D,"Received OAM cell with invalid CRC\n");
		return;
	}
...
static struct sk_buff *rawcell_decode(struct unicorn_ethdrv *drv, unsigned char *cell)
{
	...
	if (!skb) {
		...
		if (!skb) {
			DBG(ATM_D,"dev_alloc_skb failed\n");
			return NULL;
		}
		skb->dev = drv->dev;
	}
    ...
    // copy data
    	...
    	INFO("rawcell_decode:skb->len > (max_mru-ATM_CELL_PAYLOAD)\n");
		skb_trim(skb, 0);
	}
...
static struct sk_buff *aal5_decode(struct sk_buff *skb) 
{
	...
	pdu_length  = ((length + 47 + 8)/ 48) * 48;
  	...
  	if (skb->len < pdu_length) {
		DBG(ATM_D,"skb to short,skb->len=%d,pdu_length=%d\n",skb->len,pdu_length);
		goto fail;
	}
  
 	// is skb too long ? 
  	if (skb->len > pdu_length) {
		DBG(ATM_D,"readjusting illegal skb->len %d -> %d\n",skb->len,pdu_length);
...
static void rcv_aal5(struct unicorn_ethdrv *drv,unsigned char *cell)
{
	...
	if ((drv->protocol == RFC2684_BRIDGED)||(drv->protocol == RFC2684_PPPOE)) {
		// remove encaps
		if (drv->encaps == LLC) {
			if ((skb->len >= sizeof(BRIDGED_LLC_OUI_PID)) /* &&
				...
				DBG(ATM_D,"wrong BRIDGED_LLC_OUI_PID encaps %02x%02x\n",skb->data[0],skb->data[1]);
				goto fail;
			}
		} else {
			if ((skb->len >= sizeof(PAD)) &&
				...
				DBG(ATM_D,"wrong PAD encaps %02x%02x\n",skb->data[0],skb->data[1]);
				goto fail;
			}
		}
		...
		if (drv->encaps == LLC) {
			if ((skb->len >= sizeof(ROUTED_LLC_OUI_PID)) &&
				...
				DBG(ATM_D,"wrong ROUTED_LLC_OUI_PID encaps %02x%02x\n",skb->data[0],skb->data[1]);
				goto fail;
			}
		}
		// add enet header
		...
		if (drv->encaps == LLC) {
			if ((skb->len >= sizeof(PPPOA_LLC_ENCAPS)) &&
				...
				DBG(ATM_D,"wrong PPPOA_LLC_ENCAPS %02x%02x\n",skb->data[0],skb->data[1]);
				goto fail;
			}
		}
		// add PPPoE header
	...
	case NET_CTL_GET_VPI_VCI:
		{
			...
			vpi_vci->vpi = drv->vpi;
			vpi_vci->vci = drv->vci;
			vpi_vci->atm_encaps = drv->encaps;
			vpi_vci->atm_protocol = drv->protocol;
...
//	INFO("v %d.%d.%d, " __TIME__ " " __DATE__"\n",(VERS>>8)&0xf,(VERS>>4)&0xf,VERS&0xf);
	...
	if (unicorn_ethdrv) {
		DBG(ATM_D,"driver already opened\n");
		return -ENXIO;
	}
...
extern int unicorn_attach(struct unicorn_entrypoints *entrypoints)
{
	...
	if (register_netdev(drv->dev)) {
		DBG(ATM_D,"register_netdev failed\n");
		return -ENXIO;
	}
